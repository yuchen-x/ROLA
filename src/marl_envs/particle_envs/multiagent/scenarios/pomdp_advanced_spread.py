...
class Scenario(BaseScenario):
    def make_world(self, config, prob_random=0.2, obs_r=1.0, obs_resolution=8):
        """
        config - dictionary
        ...
        for i, landmark in enumerate(world.landmarks):
            if rand_num < self.prob_random:
                ...
                landmark.state.p_pos = np.array(
                    [self.landmarks_x[i], self.landmarks_y[i]]
                )
    ...
    def observation(self, agent, world):
        ...
        for other in world.agents:
            ...
            if dist <= agent.obs_range:
                others.append(
                    other.state.p_vel - agent.state.p_vel
                )  # relative velocity
                others.append(
                    other.state.p_pos - agent.state.p_pos
                )  # relative position
            else:
                others.append(np.zeros(world.dim_p))
                others.append(np.zeros(world.dim_p))
        return np.concatenate([agent.state.p_vel, agent.state.p_pos] + others)
    ...
    def state(self, world):
        ...
        return np.around(
            np.concatenate(agent_pos + agent_vel + entity_pos),
            decimals=world.agents[0].obs_resolution,
        )

    def env_info(self, world):
        # something to have in order to work with methods which requires true state
        return {"state_shape": len(world.landmarks) * 2 + len(world.agents) * 4}
